...
class YOLOV5Backbone(nn.Module):
    def __init__(self, input_channels=3, return_index=[4, 6, 9], depth_multiple=1.0, width_multiple=1.0):
        ...
        model.append(conv3)
        csp2 = C3(make_div_fun(256), make_div_fun(256), n=make_round_fun(6))  # 4 args = [128,128,2]
        model.append(csp2)
        conv4 = Conv(make_div_fun(256), make_div_fun(512), k=3, s=2)  # 5 args = [128,256,3,2]
        model.append(conv4)
        csp3 = C3(make_div_fun(512), make_div_fun(512), n=make_round_fun(9))  # 6 args = [256,256,3]
        model.append(csp3)
        ...
        model.append(csp4)
        sppf = SPPF(make_div_fun(1024), make_div_fun(1024), k=5)  # 9 args = [512,512,5]
        model.append(sppf)
