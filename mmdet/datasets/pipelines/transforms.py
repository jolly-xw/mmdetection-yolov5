...
class LetterResize(object):
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            # wh padding
            dw, dh = self.image_size_hw[1] - \
                new_unpad[0], self.image_size_hw[0] - new_unpad[1]
            if self.auto:  # minimum rectangle
                ...
                # width, height ratios
                ratio = self.image_size_hw[1] / \
                    shape[1], self.image_size_hw[0] / shape[0]
            ...
            img = cv2.copyMakeBorder(
                img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=self.color)  # add border

            results[key] = img
            # save
            # mmcv.imwrite(img, '/home/wx/git/yuml/demo/transform_images/resize.jpg')
            results['pad_shape'] = img.shape
            results['letter_pad'] = np.array(
...
class LetterBox:
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            img = results[key]
            image, shape, top, bottom, left, right, scale_factor = self.letterbox(img, self.new_hw, self.color,
                                                                                  self.auto, self.scaleFill,
                                                                                  self.scaleup, self.stride)
            results[key] = image
            results['scale_factor'] = scale_factor
    ...
    def letterbox(im, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
        ...
        return im, im.shape, top, bottom, left, right, scale_factor
...
class Pad:
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            # # save
            # mmcv.imwrite(
            #     padded_img, '/home/wx/git/yuml/demo/transform_images/pad.jpg')
...
class NormalPad:
    ...
    def normalimpad(img,
        ...
        if shape is not None:
            ...
            #            左 上   右      下
            padding = (width//2,height//2,width//2,height//2)
        # check pad_val
        ...
        img = cv2.copyMakeBorder(
            ...
            value=pad_val)
        pad_duijie = padding # 对接测试
        return img,pad_duijie
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.pad_to_square:
                ...
                self.size = (max_size, max_size)
            if self.size is not None:
                padded_img,pad_duijie = NormalPad.normalimpad(
                    results[key], shape=self.size, pad_val=pad_val)
            elif self.size_divisor is not None:
                padded_img = mmcv.impad_to_multiple(
                    results[key], self.size_divisor, pad_val=pad_val)
...
class YOLOV5HSVRandomAug:
    ...
    def __call__(self, results):
        ...
        results['img'] = img
        cv2.imwrite('demo/aug_test/hsv_test.jpg', img)
        return results
...
@PIPELINES.register_module()
class YOLOV5Albumentations:
    """
    albumentations是一种针对数据增强专门写的API,里面基本包含大量的数据增强手段,
    yolov5使用以下数据增强:
    CLANE:对比度受限自适应直方图均衡
    Blur:模糊图像
    ToGray:灰度图
    ...
    RandomBrightnessContrast:随机对比度亮度
    """
    ...
    def __init__(self):
        try:
            ...
            logging.info("albumentations"+f'{e}')
        # T = [
        #     A.Blur(p=0.01),
        #     A.MedianBlur(p=0.01),
        #     A.ToGray(p=0.01),
    ...
    def __call__(self, results):
        img = results['img']  # 在loadimage阶段mmcv.imread读进来的就是BGR
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # BGR要转成RGB
        if self.transform:
            transformed = self.transform(image=img)
            transformed_image = transformed['image']
            img = cv2.cvtColor(transformed_image,
                               cv2.COLOR_RGB2BGR)  # RGB需要再转成BGR
            results['img'] = img  # 更新results字典中的img
        return results


@PIPELINES.register_module()
class YOLOV5RandomPerspective:
    def __init__(self,
                 degrees=10,
                 translate=0.1,
                 scale=0.1,
    ...
    def _trans_bbox(self, bbox, Matrix, perspective, scale, width, height, filter=None):
        n = len(bbox)
        if n:
            new = np.zeros((n, 4))
            # warp boxes
            ...
            xy[:, :2] = bbox[:, [0, 1, 2, 3, 0, 3, 2, 1]].reshape(
            ...
            i = self.box_candidates(
                box1=bbox[:, 0:4].T * scale, box2=new.T, area_thr=0.10)
            filter = copy.deepcopy(i)
            bbox = bbox[i]
            bbox[:, 0:4] = new[i]
        return bbox, filter

    def _perspective_bbox(self, results, Matrix, height, width, perspective, scale):
        '''对bbox进行perspective变换'''
        for key in results.get('bbox_fields', []):
            bboxes = results[key]
            filter_bool = None
            results[key], filter_bool = self._trans_bbox(bboxes,
                                                         Matrix=Matrix,
                                                         perspective=perspective,
                                                         scale=scale,
                                                         width=width,
                                                         height=height)
            if key == 'gt_bboxes':  # 过滤label
                results['gt_labels'] = results['gt_labels'][filter_bool]

    def _perspective_img(self,
                         results,
                         im,
                         degrees=10,
                         translate=.1,
        ...
        # 更新图片
        results['img'] = im
        return Matrix, height, width, perspective, s

    def __call__(self, results):
        image = results['img']  # BGR
        # 1.对图片进行仿射变换
        # M为转换矩阵 h为图片高 w为图片宽 p为透视参数 s为缩放因子
        M, h, w, p, s = self._perspective_img(results,
                                              image,
        ...
        # 2.对bbox进行仿射变换
        self._perspective_bbox(results, M, h, w, p, s)
        return results
