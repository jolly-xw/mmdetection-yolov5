...
@PRIOR_GENERATORS.register_module()
class YOLOV5AnchorGenerator(YOLOAnchorGenerator):
    def __init__(self, strides, base_sizes):
        self.strides = [_pair(stride) for stride in strides]
        self.centers = [(stride[0] / 2., stride[1] / 2.)
                        for stride in self.strides]
    ...
    def find_gxy(self, grid_idx, width):
        y = grid_idx // width
        x = grid_idx - y * width
        return x, y
    ...
    def responsible_flags(self, featmap_sizes, gt_bboxes, device='cuda'):
        ...
        for i in range(self.num_levels):
            anchor_stride = self.strides[i]
            flags, bbox_targets = self.single_level_responsible_flags(
                featmap_sizes[i],
                self.base_anchors[i],
                gt_bboxes,
                anchor_stride,
                ...
                device=device)
            multi_level_responsible_flags.append(flags)
            multi_bbox_targets.append(bbox_targets)
        return multi_level_responsible_flags, multi_bbox_targets
    ...
    def single_level_responsible_flags(self,
        ...
        # ori image size
        anchor_w = (anchors[:, 2] - anchors[:, 0]).to(device)
        anchor_h = (anchors[:, 3] - anchors[:, 1]).to(device)
        gt_w = gt_bboxes[:, 2] - gt_bboxes[:, 0]
        gt_h = gt_bboxes[:, 3] - gt_bboxes[:, 1]

        # feature map size
        gx = gt_bboxes_cx / stride[0]
        gy = gt_bboxes_cy / stride[1]
        gxi = feat_w - gx
        gyi = feat_h - gy
        ...
        # responsible_grid = responsible_grid[:, None].expand(responsible_grid.size(0), num_base_anchors)
        # expand 共享内存
        responsible_grid = responsible_grid[:, None].repeat(1, num_base_anchors)
        # check
        assert responsible_grid.shape[1] == len(anchors)
        # row major indexing
        gt_bboxes_grid_idx_ = gt_bboxes_grid_y * feat_w + gt_bboxes_grid_x
        # if ignored idx for expanding pos idx
        # for each anchor idx
        for anchor_idx in range(len(anchors)):
            gt_bboxes_grid_idx = gt_bboxes_grid_idx_
            aw = anchor_w[anchor_idx] / stride[0]
            ah = anchor_h[anchor_idx] / stride[1]
            ignore_idx = []
            for i, idx in enumerate(gt_bboxes_grid_idx):
                rw = anchor_w[anchor_idx] / gt_w[i]
                rw_ = 1 / rw
                rh = anchor_h[anchor_idx] / gt_h[i]
                rh_ = 1 / rh
                ratio = max(rw, rw_, rh, rh_)
                if ratio > 4.0:
                    ignore_idx.append(int(idx))

            for i, idx in enumerate(gt_bboxes_grid_idx):
                if int(idx) not in ignore_idx:
                    flag_bbox_add_idx = False
                    if gx[i] > 1 and gx[i] % 1 < 0.5:
                        new_idx = idx - 1  # new_idx: positive anchor's idx in grid
                        x, y = self.find_gxy(new_idx, feat_w)  # get positive anchor's (x,y) in grid cell
                        target1 = torch.tensor([gx[i] - x, gy[i] - y, aw, ah])
                        x, y = self.find_gxy(idx, feat_w)
                        target2 = torch.tensor([gx[i] - x, gy[i] - y, aw, ah])
                        responsible_grid[:, anchor_idx][new_idx] = 1
                        responsible_grid[:, anchor_idx][idx] = 1
                        key = str(int(new_idx))
                        bbox_targets[key] = target1
                        if not flag_bbox_add_idx:
                            key = str(int(idx))
                            bbox_targets[key] = target2
                            flag_bbox_add_idx = True
                    if gy[i] > 1 and gy[i] % 1 < 0.5:
                        new_idx = idx - feat_w
                        x, y = self.find_gxy(new_idx, feat_w)
                        target1 = torch.tensor([gx[i] - x, gy[i] - y, aw, ah])
                        x, y = self.find_gxy(idx, feat_w)
                        target2 = torch.tensor([gx[i] - x, gy[i] - y, aw, ah])
                        responsible_grid[:, anchor_idx][new_idx] = 1
            ...
            # responsible_grid[:, anchor_idx][gt_bboxes_grid_idx[~np.isin(gt_bboxes_grid_idx.to('cpu'), ignore_idx)]]
            # = 1

        # responsible_grid = responsible_grid[:, None].expand(
        #     responsible_grid.size(0), num_base_anchors).contiguous().view(-1)
        responsible_grid = responsible_grid.contiguous().view(-1)
        return responsible_grid, bbox_targets
